...
function addTodo() {
  ...
  const input = prompt("Enter Your To Do List: ").trim();
  // 2. Validasi input: Pastikan teks tidak kosong atau hanya spasi
  if (!input) {
    console.log("To do can not be empty");
    return;
  }
...
function markTodoCompleted() {
  ...
  console.log(`to-do "${todo.text}" mark as completed`);
  // 6. Tangani kasus jika to-do sudah selesai
  console.log(`to-do "${todo.text}" is done`);
}
...
function deleteTodo() {
  ...
  if (todos.length === 0) return;
  const answer = prompt("Enter the NUMBER you want to be delete");
  const index = parseInt(answer) - 1;
